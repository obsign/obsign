//! PKCS#11 sealer: the production implementation of [`Sealer`].
//!
...
//! story; if one is ever wanted, it is another implementation of the same
//! trait, not a change here.
//!
//! The bindings are hand-rolled over `dlopen`, in the spirit of the HTTP and
//! DER code: the seven calls sealing needs, not a binding crate that drags
...
//! the checkpoint honestly summarizes the WAL — that stays the ledger's job.
//! What the HSM buys is that a compromised ledger host can sign *now* but
//! cannot exfiltrate the key and sign *later, offline, at leisure* — history
//! rewritten after the compromise is discovered still fails verification.
...
impl Pkcs11Sealer {
    ...
    pub fn open(
        ...
        std::mem::forget(guard); // the session now belongs to the sealer
        Ok(Pkcs11Sealer {
            funcs,
            session,
            private_key,
...
impl std::fmt::Debug for Pkcs11Sealer {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Pkcs11Sealer")

...
use obsign_audit_core::record::{Payload, Record};
use obsign_ledger::Store;
...
fn chain_row(console: &Console, id: &str) -> ChainRow {
    let records = match obsign_wal::read(&console.wal_dir, id) {
        Ok(r) => r,
        Err(e) => {
            return ChainRow {
                id: id.to_string(),
    ...
    match store {
        ...
        Some(store) => {
            ...
            let evidence = obsign_ledger::export(records, &store, &[], None);
            let report = obsign_audit_core::evidence::verify(&evidence, &trusted);
...
fn release_page(console: &Console) -> Option<String> {
    ...
    if let Ok(raw) = std::fs::read_to_string(&bundle_path) {
        match serde_json::from_str::<obsign_policy::SignedBundle>(&raw) {
            Err(e) => {
                let _ = write!(
                    body,
    ...
    if let Ok(raw) = std::fs::read_to_string(dist.join("identity-bundle.json")) {
        if let Ok(signed) = serde_json::from_str::<obsign_identity::SignedIdentityBundle>(&raw) {
            let b = &signed.bundle;
...
fn chain_page(console: &Console, id: &str) -> Option<String> {
    let records = obsign_wal::read(&console.wal_dir, id).ok()?;
    ...
    match &rec.payload {
        ...
        Payload::ConfigReload(c) => {
            let mut s = format!(
                "<b>{}</b> {} — in force: {}",
                match c.status {
                    obsign_audit_core::record::ReloadStatus::Applied => "applied",
                    obsign_audit_core::record::ReloadStatus::Rejected => "REJECTED",
                },

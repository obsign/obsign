...
pub enum Error {
    ...
    #[error("policy: {0}")]
    Policy(#[from] obsign_policy::Error),

    #[error("identity: {0}")]
    Identity(#[from] obsign_identity::Error),

    #[error("ledger: {0}")]
    Ledger(#[from] obsign_ledger::Error),

    #[error("log: {0}")]
    Wal(#[from] obsign_wal::Error),

    #[error(transparent)]
    Core(#[from] obsign_audit_core::Error),

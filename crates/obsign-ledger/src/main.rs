...
use obsign_audit_core::rfc3161::{parse_timestamp_response, Anchor};
...
fn make_sealer(args: &SealArgs) -> Result<Box<dyn Sealer>> {
    match (&args.key, &args.hsm_module) {
        ...
        #[cfg(unix)]
        (None, Some(module)) => {
            use obsign_ledger::{Pkcs11Sealer, TokenSelector};
            let label = args
                .hsm_key_label
                .as_deref()
...
fn read_keys(path: &Path) -> Result<Vec<obsign_audit_core::checkpoint::PublicKeyEntry>> {
    let raw = std::fs::read_to_string(path)
        .with_context(|| format!("reading {}", path.display()))?;
    serde_json::from_str(&raw).with_context(|| format!("parsing {}", path.display()))
}

fn origin_policy(args: &SealArgs) -> Result<obsign_ledger::OriginPolicy> {
    ...
    if let Some(bundle_path) = &args.deployment_bundle {
        ...
        let signed: obsign_audit_core::SignedDeploymentBundle = serde_json::from_str(&raw)
        ...
        let policy = obsign_ledger::OriginPolicy::from_bundle(&signed, &ops_vk, require)?;
        eprintln!(
    ...
    match &args.trusted_origin_keys {
        None => {
            ...
            Ok(obsign_ledger::OriginPolicy::permissive())
        }
        Some(path) => {
            ...
            Ok(obsign_ledger::OriginPolicy::new(&keys, args.require_origin)?)
        }
    }
}
...
fn one_pass(
    args: &SealArgs,
    sealer: &dyn Sealer,
    origin: &obsign_ledger::OriginPolicy,
) -> Result<bool, obsign_ledger::Error> {
    let records = obsign_wal::read(&args.wal, &args.store.chain_id)?;
...
fn run_loop(
    ...
    loop {
        match one_pass(args, sealer, origin) {
            ...
            // orchestrator alerts.
            Err(
                e @ (obsign_ledger::Error::DivergedLog { .. }
                | obsign_ledger::Error::TruncatedLog { .. }
                | obsign_ledger::Error::StoreBroken(_)
                | obsign_ledger::Error::KeyConflict(_)
...
fn do_export(
    ...
    let records = obsign_wal::read(wal_dir, &store_args.chain_id).context("reading the log")?;
    let store =

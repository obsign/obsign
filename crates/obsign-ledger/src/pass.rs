...
use crate::Error;
use obsign_audit_core::checkpoint::{seal_interval, KeyRole, PublicKeyEntry, SignedCheckpoint};
use obsign_audit_core::deployment::SignedDeploymentBundle;
use obsign_audit_core::evidence::{Evidence, FORMAT};
use obsign_audit_core::origin::SignedRecord;
use obsign_audit_core::record::Record;
        ...
        for sr in records {
            if let obsign_audit_core::record::Payload::SessionCert(cert) = &sr.record.payload {
                if let Some(id_vk) = self.trusted.get(&cert.identity_key_id) {
                    if let Ok(vk) = obsign_audit_core::verify_session_cert(chain_id, cert, id_vk) {
                        m.insert(obsign_audit_core::key_id_for(&vk), vk);
                    }
                }
            }
        }

...
/// obsign-audit-core's `#[cfg(test)]` helper on purpose: a forgery helper must not
...
fn seal_export_verify_roundtrip() {
    ...
    let s = sealer();
    let records = obsign_wal::read(&wal_dir, "c1").unwrap();
...
fn rewritten_wal_is_refused_before_any_new_seal() {
    ...
    assert!(
        matches!(err, obsign_ledger::Error::DivergedLog { seq: 4 }),
        "got {err:?}"
    );
...
fn wal_shorter_than_sealed_history_is_refused() {
    ...
    let err = seal_pass(&records, &mut store, &s, &no_origin(), 2000, 1).unwrap_err();
    assert!(
        matches!(err, obsign_ledger::Error::TruncatedLog { sealed_to: 4, .. }),
        "got {err:?}"
    );
...
fn edited_store_refuses_to_open() {
    ...
    assert!(
        matches!(err, obsign_ledger::Error::Core(obsign_audit_core::Error::BadSignature { .. })),
        "got {err:?}"
    );
...
fn removed_checkpoint_breaks_the_store_chain() {
    ...
    let err = Store::open(&store_dir, "c1").unwrap_err();
    assert!(matches!(err, obsign_ledger::Error::StoreBroken(_)), "got {err:?}");
...
fn a_key_id_cannot_be_rebound_to_another_key() {
    ...
    assert!(matches!(err, obsign_ledger::Error::KeyConflict(_)), "got {err:?}");
...
fn anchor_roundtrip_reaches_the_evidence_pack() {
    ...
    store
        .append_anchor(obsign_audit_core::rfc3161::Anchor {
...
fn foreign_or_orphan_tokens_do_not_attach() {
    ...
    assert!(matches!(err, obsign_ledger::Error::AnchorMismatch(_)), "got {err:?}");

    // Token for a checkpoint this store does not hold: refused.
    let ghost = obsign_audit_core::Hash([0x42; 32]);
    let resp = tsa::granted_response(ghost.as_bytes());
    let err = validate_response(&store, &ghost, &resp).unwrap_err();
    assert!(
        matches!(err, obsign_ledger::Error::UnknownCheckpoint(_)),
        "got {err:?}"
    );
...
mod origin {
    ...
    fn append_forged(dir: &Path) {
        let records = obsign_wal::read(dir, "c1").unwrap();
    ...
    fn a_forged_tail_is_never_sealed_and_raises_the_alarm() {
        ...
        match err {
            obsign_ledger::Error::UnauthenticatedRecord {
                seq,
                prefix_sealed_to,
                ..
            } => {
        ...
        assert!(matches!(
            err,
            obsign_ledger::Error::UnauthenticatedRecord { seq: 4, prefix_sealed_to: None, .. }
        ));
    ...
    fn an_invalid_signature_under_a_trusted_key_refuses_even_in_rollout_mode() {
        ...
        assert!(
            matches!(err, obsign_ledger::Error::UnauthenticatedRecord { seq: 1, .. }),
            "got {err:?}"
        );
    ...
    fn rollout_mode_seals_unsigned_records() {
        ...
        assert!(matches!(
            err,
            obsign_ledger::Error::UnauthenticatedRecord { seq: 3, .. }
        ));
    ...
    fn origin_trust_resolves_from_a_signed_deployment_bundle() {
        ...
        let other_entry = obsign_audit_core::checkpoint::PublicKeyEntry {
            ...
            role: obsign_audit_core::checkpoint::KeyRole::Origin,
        };
        ...
        assert!(
            matches!(err, obsign_ledger::Error::UnauthenticatedRecord { seq: 3, .. }),
    ...
    fn the_trusted_origin_file_refuses_sealing_keys() {
        ...
        assert!(matches!(err, obsign_ledger::Error::StoreBroken(_)));
    }
}

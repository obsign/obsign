...
use obsign_ledger::{export, seal_pass, Pkcs11Sealer, Sealer, TokenSelector};
...
fn grow_wal(dir: &Path, n: u64) {
    ...
    for i in start..start + n {
        ...
        wal.append(&obsign_audit_core::SignedRecord::unsigned(r)).unwrap();
    }
}
...
fn seals_through_a_real_token() {
    ...
    let mut store = obsign_ledger::Store::open(&tmpdir("store"), "c1").unwrap();

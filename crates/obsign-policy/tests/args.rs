...
#[test]
fn smoke_check_names_the_rule_with_the_typo() {
    let cedar_typo = r##"
    @id("typo")
    forbid (principal, action == Action::"tool_call", resource)
    ...
    let err = e.smoke_check().err().unwrap().to_string();
    assert!(err.contains("typo"), "the rule id must be named: {err}");
    assert!(err.contains("send_message"), "{err}");

    // The healthy engine passes — including the tools without declarations.
    assert!(engine().smoke_check().is_ok());
}

#[test]
fn smoke_check_ignores_rules_scoped_to_other_tools() {
    // `search` declares nothing; the channel rule is scoped to
    // `send_message`. Cedar short-circuits the scope before the condition,
    // so evaluating `search` must not trip over `context.args.channel`.
    let e = Engine::load(&bundle_v2(
        CEDAR,
        vec![
            tool_with_args("send_message", vec![arg("channel", ArgKind::String)]),
            tool_with_args("search", vec![]),
        ],
    ))
    .unwrap();
    assert!(e.smoke_check().is_ok());
}

// ---- catalogue-time validation -------------------------------------------

#[test]
fn v1_bundle_with_policy_args_is_refused_at_load() {
    let mut b = bundle_v2(
        CEDAR,
        vec![tool_with_args(
            "send_message",
    ...
    let err = Engine::load(&b).err().unwrap();
    assert!(err.to_string().contains("obsign-policy/2"), "{err}");
}

#[test]
fn v1_bundle_with_policy_args_fails_signature_verification() {
    // v1 signing bytes do not cover the declarations: a signed v1 bundle
    // with policy_args injected after signing must not verify.
    ...
    let clean = Bundle {
        ...
        version: "policies@test".into(),
        cedar: r#"@id("allow_all") permit (principal, action, resource);"#.into(),
        tools: vec![tool_with_args("send_message", vec![])],
        fail_mode: FailMode {
            default: FailBehaviour::Closed,
            tools: Default::default(),
        },
    };
    let mut signed = clean.sign("ops", &key);
    // The injection: same signature, declarations added afterwards.
    signed.bundle.tools[0].policy_args = vec![arg("channel", ArgKind::String)];
    assert!(signed.verify(&key.verifying_key()).is_err());
}

#[test]
fn v2_signature_covers_the_declarations() {
    use ed25519_dalek::SigningKey;
    let key = SigningKey::from_bytes(&[0x42; 32]);
    ...
    let signed = b.sign("ops", &key);
    assert!(signed.verify(&key.verifying_key()).is_ok());

    // Tampering with a declaration after signing breaks the signature.
    let mut tampered = signed.clone();
    tampered.bundle.tools[0].policy_args[0].default = Some(json!("#anywhere"));
    assert!(tampered.verify(&key.verifying_key()).is_err());
}

#[test]
fn bad_declarations_fail_at_load_not_at_call_time() {
    // Duplicate name.
    let b = bundle_v2(
        CEDAR,
        vec![tool_with_args(
            "send_message",
            vec![arg("channel", ArgKind::String), arg("channel", ArgKind::Bool)],
        )],
    );
    assert!(Engine::load(&b).err().unwrap().to_string().contains("duplicate"));

    // Pointer that is not a pointer.
    let mut spec = arg("channel", ArgKind::String);
    spec.at = Some("message.channel".into());
    let b = bundle_v2(CEDAR, vec![tool_with_args("send_message", vec![spec])]);
    assert!(Engine::load(&b).err().unwrap().to_string().contains("JSON pointer"));

    // Default that does not coerce under its own kind.
    let mut spec = arg("amount_cents", ArgKind::Long);
    spec.default = Some(json!("free"));
    let b = bundle_v2(CEDAR, vec![tool_with_args("transfer", vec![spec])]);
    assert!(Engine::load(&b).err().unwrap().to_string().contains("default"));

    // Too many declarations.
    let many: Vec<ArgSpec> = (0..17).map(|i| arg(&format!("a{i}"), ArgKind::Bool)).collect();
    let b = bundle_v2(CEDAR, vec![tool_with_args("send_message", many)]);
    assert!(Engine::load(&b).err().unwrap().to_string().contains("maximum is 16"));
}

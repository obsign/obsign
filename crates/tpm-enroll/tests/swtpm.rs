...
//! `audit_core::verify_attestation` accepts the output, byte for byte.
...
use audit_core::attestation::verify_attestation;
use audit_core::error::Error as CoreError;
use std::net::{TcpListener, TcpStream};
use std::path::PathBuf;
use std::process::{Child, Command, Stdio};
use std::time::{Duration, Instant};
use tpm_enroll::ctrl::SwtpmCtrl;
use tpm_enroll::{enroll, EnrollmentRequest, KeyAlg, Tpm};

fn swtpm_on_path() -> bool {
    Command::new("swtpm")
        .arg("--version")
        .stdout(Stdio::null())
        .stderr(Stdio::null())
...
fn start_swtpm() -> Swtpm {
    for attempt in 0..3 {
        let state =
            std::env::temp_dir().join(format!("probant-swtpm-{}-{attempt}", std::process::id()));
...
fn enrolls_against_a_real_software_tpm_and_the_verifier_accepts_it() {
    ...
    assert_eq!(
        ...
        hex::encode(audit_core::hash::sha256(&preimage).as_bytes()),
    ...
    // KeyAttestation. Both must deserialize into audit-core's types and
    ...
    let attestation: audit_core::attestation::KeyAttestation =

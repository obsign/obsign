...
answers: **who is told?** Not by the ledger itself, which makes no network
...
`obsign-alert@.service` is a template unit, instantiated with the name of the
...
or stays on the management network, with no HTTP and no cloud endpoint. The
journal entry is the minimum; keep whichever other channels the site already
monitors:
...
The ledger's refusal message says which sequence number diverged and whether
an authentic prefix was sealed. It is on stderr, hence in the journal of the
failed unit, and the alert only has to point at it.

## Slack, without breaking the air gap

A chat alert is worth having, and it does not contradict the design, as long
as the same rule holds: **the network call lives in the supervisor.** The
ledger still makes none. Slack is reached, when it is reached at all, from
the alert unit, over whatever egress the enclave already permits.
`scripts/obsign-slack-alert.sh` posts to a Slack Incoming Webhook under three
constraints that keep it honest in an air-gapped site:

- **Best-effort, never load-bearing.** It runs *alongside* the local
  channels, never instead of them: journal, `wall` and mail are the
  guarantee, and they always deliver inside the enclave. Slack is a
  convenience on top. If it cannot be reached the script logs a warning and
  exits `0`; an alert channel must never become the incident.
...
proxy, in an `EnvironmentFile` the alert unit reads, so that the URL stays out
of `ps` and the unit text:
...
Validate the wiring offline before an incident ever fires. The dry run builds
the exact payload and names the target and proxy without posting:
...
after the first incident until an operator explicitly clears it. That is the
right posture anyway, since divergence never self-heals and the halted state
is exactly what the investigation wants to look at.
...
The distroless ledger image has no systemd inside, so keep the supervisor
...
Treat it as an incident. The WAL and the ledger store are the evidence; do
not "fix" them to get sealing green again. `obsign-ledger export` still works
and writes the pack even when it fails verification: a failing pack on disk
is exactly what the investigation wants. The last sealed checkpoint (and its
RFC 3161 anchor, if any) bounds when the rewrite happened: everything up to
`to_seq` is still provable, and that boundary is the starting point of the
timeline.

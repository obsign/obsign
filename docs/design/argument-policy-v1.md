...
> whatever guards the rule carries, instead of only when a synthetic
...
server" (step 2); compile-time checks, namely `Engine::smoke_check` and v2
emitted only when a tool declares args (step 3); the demo, with
`send_message` + `support_channel_only` in the mkbundle reference policy,
argument refusal in `run-demo.sh`, and the flow verified live end-to-end
including the sealed pack (step 4); docs covering the README
demo/decision/format sections and the v1-to-v2 cutover rule in
`deploy-docker.md` (step 5). **All five steps landed.** One point was
adjusted on contact with the code and is now §3.8: the `tools/list` filter
reused `evaluate`, which would have *hidden* every argument-restricted
tool, so the listing gets its own entry point. Closes the gap between what
the policy engine arbitrates (tool × identity × context) and where
real-world refusals actually live (this tool, but not with *these*
arguments). Prompted by a design-partner question: "how do I allow
`send_message`, but only to `#support`?" Today the honest answer is "split
the tool at the MCP server", which does not survive third-party servers.
...
`obsign-proxy/src/gateway.rs`). Both facts are deliberate, since arguments
carry personal data and the log therefore keeps only their hash. But the
...
1. **The dominant class of real refusals is inexpressible.** "`query` but
   ...
   "`transfer` under 10 000 €" are all argument constraints. Tool-level
   policy can only say yes or no to the verb, never to the object.
...
- Cedar needs types. JSON reaches the gateway untyped and
  ...
  arbitrary nesting.
- Data minimisation is structural. The bundle is the written,
...
- Evaluation stays bounded. Policies never traverse unbounded input; caps
...
sent, and the audit chain would prove a lie. Refusal is the only honest
verdict.
...
- `long` accepts integral JSON numbers in `i64` range only. Floats are
  refused; an amount check that rounds is an amount check with a hole.
  Monetary rules that need decimals declare minor units (cents); the doc
  comment states that convention, and float comparison does not substitute
  for it.
...
- an argument present in the call is extracted and coerced per `kind`;
- one absent with a `default` receives the default;
- one absent without a default means the call is refused before Cedar
  (§3.3).

An explicit `null` counts as absent, so the default applies or the call is
refused. MCP client SDKs routinely serialize an omitted optional field as
...
2. **Evaluation** can still fail on a *well-typed* value that makes a policy
   expression unevaluable (i64 arithmetic overflow, `ip()`/`decimal()` over
   a non-parsing string). For a tool that declares arguments, that Cedar
...
- ≤ 4 KiB per extracted string, ≤ 64 elements per set (call-time, refusal
  ...
  paths, amounts — and never payloads; the cap is a statement of that, and
  it bounds what a hostile agent can make Cedar chew on (`like` over
  megabytes).
...
Bumping was forbidden once before, for evidence packs, where a bump strands
sealed history. That precedent does not apply here: bundles are not
historical artifacts, they are re-minted from git at will
(`policies@<sha>`), and what the log engraves is the version string and rule
ids, leaving the bundle encoding out of the record.

Compatibility matrix, all paths loud:

- a new gateway with a v1 bundle loads, with no argument rules (empty
  `policy_args` everywhere);
- an old gateway with a v2 bundle raises `UnknownFormat` at startup and
  refuses to run. Correct: a stale gateway silently enforcing *less* than
  the bundle says would be the worst outcome. Upgrade gateways before
  publishing v2 bundles; the control plane can keep emitting v1 until the
  fleet is ready (emit v2 only when some tool declares `policy_args`, which
  is mechanical and makes the cutover self-serve).
...
- The known cost, stated: an argument-dependent verdict is **attested but
  ...
  `args_hash`). That is a separate design, to be taken up when a partner's
  auditor actually requires replay and not before.
...
- The permissive bias is safe by construction: the listing is hygiene, and
  ...
  cosmetic, since a tool whose only permits are argument-dependent shows up
  in the list even for an agent whose every call will be refused.
...
tests so they stay conscious choices instead of surprises:
...
- **The leniency is not scoped to argument errors.** Any evaluation error
  ...
  is dropped, whereas the call path would escalate it to the fail mode. So
...
- **Only `forbid`-style argument rules keep a tool listed.** A tool whose
  *only* permit is argument-conditional (`permit ... when {
  ...
  permit dropped on the listing path, so the decision falls to default-deny
  and the tool is hidden, though conforming calls succeed. Write argument
  restrictions as a `forbid` paired with a broad `permit` (as the reference
  policy does); a permit that gates visibility on `context.args` is the
  shape to avoid. Pinned by
  `permit_only_argument_rules_hide_the_tool_from_listings`.

## 4. What this deliberately does not fix

1. **The request as written.** The policy matches the argument the
...
2. **The content of an allowed argument.** A prompt-injection payload
   *inside* an allowed argument (`message` text sent to the allowed
   `#support`) passes. Judging content is a different product surface (and
   a different failure mode: probabilistic, where this engine is
   deliberately not).
...
2. **obsign-proxy**: hand `params.arguments` to the engine and nothing
   else, since extraction lives in one place. e2e: a call refused *on its
   arguments* never reaches the server (extend the existing never-reached
   assertion); http.rs: same through the HTTP transport.
...
4. **mkbundle + docker demo**: give the reference policy the
   ...
   `send_message` to a forbidden channel, so the argument refusal becomes
...
5. **README + docs**: the policy section gains the declaration/rule pair
   from §2; `deploy-docker.md` mentions the v1-to-v2 cutover rule.

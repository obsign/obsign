...
Decision (1) went to option (b). The gateway loads the deployment bundle,
accepts any *currently-active* origin key on resume (closing the
rotation-mid-chain gap v0 §3.5 left open), and records
`ConfigKind::DeploymentBundle` at the top of every chain;
`Wal::open_authenticated` now takes the active key *set*.

Decision (3) is done: the ops-signed bundle is embedded in the evidence pack
(`Evidence.deployment`, `#[serde(default)]`), so the auditor verifies the
whole origin chain of trust from the ops key alone.

Decision (2) is deferred. The `obsign-identity/2` claims machine-marker fix
is a separable HIGH: the deployment bundle introduces a *new* artifact and
leaves the identity format where it is, so it does not force the identity
change. It is left as its own next increment.
...
Two operational facts make this urgent:
...
- A deployment has **many gateways**, each with its own origin key. The flat
  ...
  startup, which is error-prone and unverifiable after the fact.
- Keys get compromised and gateways get decommissioned. "Stop trusting this
  key everywhere, provably, dated" is a control-plane operation that no `sed`
  on a text file can stand in for.

## 2. Decision

**A new control-plane-signed artifact, `obsign-deployment/1`, listing the
active gateway origin keys. It rides the existing pipeline from `compile` to
`publish`, and the existing ops-key trust root. The ledger and the offline
...
format changes. This is a distribution-and-trust-root change only, and it
preserves the property §7 exists to protect.
...
plus the immutable release directories. These are the properties a rule
change already has.

A bespoke enrollment protocol, in which gateways POST their public key to the
control plane, was rejected. It would be the first network-writing path in a
...
Leaving the ledger's trust as a flat file, merely signed, was rejected too.
Signing the flat file is most of the work of a real artifact without the
version, the immutable history, or the reuse of the publish pipeline. Do it
once, properly.
...
- `signing_bytes` follows the identity bundle exactly: format, version, then
  ...
  key_id, signature }` is verified under the ops verifying key. It is a
  deliberate byte-for-byte clone of `SignedIdentityBundle`.
- It does not live in the `obsign-identity` crate next to `IdentityBundle`,
  because `obsign-identity` is about *humans* and the IdP and should not gain
  a concept about *gateways*. Proposed home: `obsign_audit_core::deployment`,
  since obsign-audit-core already owns `PublicKeyEntry` and `KeyRole`, and
  both the ledger and the verifier depend on it, which is where the
  resolution logic must sit under the single-implementation-of-the-proof
  rule.
...
`releases/<sha>/`, never the current active set. So:
...
- a compromised key that signed records *before* revocation which then got
  sealed cannot be un-sealed. That is key compromise, and no revocation
  scheme repairs it. Revocation bounds the future, and the git history dates
  the boundary.
...
`compile` validates it with the same rigour it applies to the JWKS, moving
the checks that would otherwise blow up at the ledger into CI, where the PR
author is looking:
...
- an empty or absent file is allowed and means "no gateway trusted yet",
  which is legitimate before the first gateway is enrolled and more honest
  than a silent empty set (the ledger under default-require will then refuse
  to seal anything, which is correct: no trusted writer, no proof).
...
others. Zero new machinery in `release.rs`, which already iterates an
artifact list.
...
- new findings: `deployment_bundle_invalid` (**error**: bad signature, or
  ...
  key was supplied to anchor it, the self-referential case already covered by
  the `keys_not_anchored` precedent).

`verify_with` grows an optional `deployment: Option<&SignedDeploymentBundle>`
input, unless the bundle travels embedded in the pack (see open decision 3).
The v0 `VerifyOptions`/`records_origin_ok` machinery is untouched; origin
keys now have two possible provenances (explicit `--trusted-keys` entries, or
the verified bundle), unioned before the origin pass.
...
On the ledger, with a `--deployment-bundle` present, `require` defaults to
...
configuration at all (neither bundle nor flat file), behaviour is unchanged,
so pre-origin deployments are not force-upgraded by a version bump.

In the verifier, `--require-origin` becomes the default and
`--allow-unsigned-legacy-chains` downgrades `origin_unverified` back to a
warning. The person accepting the asterisk types the flag; everyone else gets
the strict verdict. `--strict` already fails on warnings, so nothing
regresses for callers who were strict.
...
Per-key validity windows and anchored-time key checks stay out. The bundle is
a membership set and says nothing about schedules. Time-bounded keys belong
with v2's session certificates, which already carry `not_before`/`not_after`
and are the natural place to verify against anchored RFC 3161 time. A
membership set plus the dated git history covers v1's revocation need.

Hardware custody and two-tier keys are still v2 (§7.1). The gateway holds a
file seed; the bundle distributes the *public* halves. The custody boundary
(`OriginSigner`) does not move.

Automatic enrollment stays out too: enrollment is a reviewed commit, by
design (§2).
...
4. **obsign verify**: `--deployment-bundle`, the two new findings, the chain
   from ops key to bundle to origin key, default `--require-origin` +
...
5. **Docs**: an operator runbook covering enroll, rotate, revoke, and *when
   to flip* (the last unlovely-flag removal is per-deployment, stays in
   prose).
...
1. **ConfigKind::DeploymentBundle + who logs the reload.** §7.2 wanted bundle
   ...
   - **(a) Defer it.** The audit trail already exists out-of-chain: git
     ...
     without a chain record. *Recommended for v1*, since it keeps the gateway
     free of a concept it does not otherwise use.
   - **(b) Give the gateway the bundle for rotation-on-resume.** Point the
     ...
     tail signed by any *currently-active* origin key instead of only its own

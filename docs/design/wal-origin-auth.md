...
- A signature whose key is simply *absent from the trusted set* counts as
  **unverified** (warning, upgraded by `require`), where §3.6 first proposed
  the hard `origin_invalid` error. Adversarially it is indistinguishable
  from an absent signature, since an attacker gains nothing by attaching an
  unresolvable one, and a hard error would break every export in the rollout
  window where gateways sign before ledgers are configured. The hard error
  is reserved for what only tampering can produce: a bad signature under a
  *trusted* key, a role-confused key, half a signature/key-id pair.
- `obsign-ledger export` (and `seal`/`run`) takes `--trusted-origin-keys`;
  ...
  at startup; that is the operator's copy-paste path into that file.
...
- The signature travels as sibling fields in the serde envelope (WAL line and
  ...
  parsing; an absent signature reads as an absent proof and never as a
  format break, the same rule the pack already applies to `anchors`.
...
non-repudiation between the two components. The entire point of the ledger
split is that neither host has to be trusted with the other's authority.
Ed25519 is already in the dependency tree; signing costs ~20 µs next to an
fsync we already pay (~tens of µs on NVMe). Three records per tool call
means <100 µs added to an operation that includes a network round trip to
the tool. Asymmetric wins.
...
since records after the last attestation are exactly the post-session-append
...
- `chain_id` is length-prefixed through the canonical `Encoder`, then the
  ...
  cross-chain transplantation (the record itself does not carry its chain
  id; the filename does, and filenames are attacker-writable).
- Signing the 32-byte digest, and never the full record, keeps the signer
...
- MVP: `FileOriginSigner`, a 32-byte hex seed file in the same custody class
  as `FileSealer`, acceptable for development and first design partners.
...
hardware). It is not, and cannot be, a defense against a compromised gateway
process. The gateway *is* the origin; origin authentication cannot defend
against the origin. That residual is stated openly.
...
- With every signature good, the pass seals as today.
- A record refused at seq *k* makes the ledger **seal the authentic prefix
  `[from..k-1]`, then return a loud error naming seq *k***
  (`Error::UnauthenticatedRecord`, which also names the prefix's upper bound
  so the operator sees both facts in one line). The prefix seals even below
  the `min_new` batching floor: those records' path to proof must not wait
  on an attacker.
  *(Implementation note: "refused" means no signature under `require`, an
  unresolvable key under `require`, or, always and even in rollout mode, an
...
detected, attributable event. The error is the alarm; `run`-mode passes
surface it on every iteration.
...
The owning, resuming path `Wal::open` (never the read-only `wal::read`)
...
refuses the open (`Error::ForeignRecord { seq }`), with no trimming and no
adoption. A record on disk that the gateway did not write is an incident for
a human to decide on, well outside the class of recoverable I/O hiccups.

Verification cost is ~50 µs/record once, at open, on a path that already
re-hashes every record; chains are session-sized, so this is not a concern.

Edge: key rotation while a chain is live means a tail signed by the previous
key. The MVP keeps this simple, letting the signer config list previous
public keys as still-verifiable, but rotation mid-chain should simply be
discouraged: chains are session-scoped and short-lived, so rotate between
sessions.
...
- `origin_invalid` (**error**): a signature present but cryptographically
  ...
  Always an error, since only tampering produces these.
...
forgeries: (a) defense in depth, since a compromised *ledger host* config
...
(3) still visible offline (3.6) if both those hosts are misconfigured.
...
- **Origin key exfiltration.** See the trust note in 3.3; a roadmap item
  beyond the MVP.
...
seal-side tests, including a forged tail after the sealed head that leaves
the prefix sealed and the alarm raised), and `e2e.rs` (the real binary
signs; the public entry is parsed off stderr exactly as an operator would;
sealed and verified under `--require-origin`). The live attack was also run
by hand against the built binaries: a fabricated record with a correctly
recomputed hash chain (the pre-v0 exit-0 scenario) now yields seal exit 1,
verify `--require-origin` exit 1 ("TAMPERING DETECTED"), and a gateway
restart that refuses to adopt the tail.
...
2. **wal**: envelopes in `append`/`replay`; `Wal::open` verification +
   ...
   dependency-light (it takes a `&dyn` or a closure, never a key file).
3. **obsign-proxy**: `OriginSigner` + `FileOriginSigner`, `Session::write`
   signs before `wal.append` (durability order unchanged: chain, sign,
   fsync, then forward). Config: `--origin-key` / env.
4. **ledger**: trusted-origin-keys config, 3.4 in `seal_pass` with
   prefix-seal semantics, `--require-origin`. Tests: a forged tail after the
   sealed head seals the prefix and raises `UnauthenticatedRecord`; e2e: a
   fabricated record can no longer reach a sealed checkpoint.
...
1. **Key custody at MVP** — seed file (proposed, matches `FileSealer`
   ...
   end state (§7.1) is not "keychain" anyway but a hardware identity key
...
2. **Origin public-key distribution** — static trusted file on the ledger
   ...
   mints authority, exactly the identity/policy bundle precedent. It wants
   the format-rev work already deferred from the claims fix; bundle both
   there.
...
### 7.1 Custody: two-tier keys

The naive end state, "the TPM signs every record", is wrong on the hot path:
...
- **Session origin key** — generated in memory at session open, never
  ...
  **session certificate** over it at open: canonical encoding, own domain
  ...
  not_after}`. Records are signed by the session key at the same ~20 µs as
  the MVP, because it is the same code path with a different key provenance.

What this buys, in threat-model terms: no origin key material ever reaches
disk, so the MVP residual (disk-reader forges) disappears entirely. The
compromise window shrinks to one session, since a leaked session key forges
one chain until its `not_after` and the identity key it would take to mint
new certificates cannot leave the hardware. Chain binding gets stronger too,
because the certificate names `chain_id`: a fabricated chain cannot obtain a
certificate for its chain id without the hardware key, so whole-chain
forgery (the post-session attack's big brother) fails at the certificate,
one level above the record.

The verifier's chain of trust becomes the control-plane root, then the
deployment bundle (§7.2), then the gateway identity key, then the session
certificate, then the per-record signature. The pack carries the certificate
alongside the records; certificate validity is judged against **anchored
RFC 3161 time** whenever an anchor is present, since the verifier's own
clock means nothing for a pack read years later.
...
- Reload of the bundle on the ledger/gateway is **recorded in the chain**
  (`ConfigReload` with a new `ConfigKind`, additive and the sanctioned
...
- legacy unsigned chains need an explicit, ugly
  `--allow-unsigned-legacy-chains`, so the asterisk gets typed by the person
  accepting it and stops being silently carried by everyone.

The same inversion eventually applies to `--trusted-keys` (gap #4): the
self-referential mode should be demoted to a flagged exception.
...
building only when a design partner's regulator asks the question. The §7.1
architecture is deliberately one signed structure away from it: the
...
- **v2:** hardware identity key + per-session certified memory keys (§7.1);
  the seed file dies.
- **v3 (on demand):** attestation quotes in enrollment (§7.5).
...
in place of a single `--origin-pubkey` flag.
